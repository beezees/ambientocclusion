...
//
// if reverse is true, this is in fact remove border
//
void
Voxels::add_border(bool reverse)
{
  cout << "Voxels::add_border(" << reverse << ")" << endl;

  int new_depth, new_width, new_height;
  if (reverse) {
    new_depth = depth - 2;
    new_height = height - 2;
    new_width = width - 2;
  ...
  int new_wxh = new_width * new_height;
  int new_size = new_depth * new_height * new_width;
  cout << "  old dimensions (d x h x w): " << depth << " x " << height << " x " << width << endl;
  cout << "  new dimensions (d x h x w): " << new_depth << " x " << new_height << " x " << new_width << endl;
  
  VoxelType *new_voxels = new VoxelType[new_size];
  assert(new_voxels);
  
  memset(new_voxels, 0, new_size * sizeof(VoxelType));
  ...
  int index = 0;
  for(int i=start_delta; i < depth - end_delta; i++) {
    int new_i = i + 1;
    if (reverse) new_i = i - 1;
    
    for(int j=start_delta; j < height - end_delta; j++) {
      int new_j = j + 1;
      if (reverse) new_j = j - 1;
      
      int new_k = 1;
      if (reverse) new_k = 0;
      int new_index = new_i * new_wxh + new_j * new_width + new_k;
      index = i * wxh + j * width + start_delta;
      
      for(int k=start_delta; k < width - end_delta; k++) {
	new_voxels[new_index] = voxels[index];
	//	new_types[new_index] = types[index];
	// if (voxels_copy) new_copy[new_index] = voxels_copy[index];
	// if (types_copy) new_types_copy[new_index] = types_copy[index];
	new_index++;
	index++;
      }  // for
    }  // for
  ...
  // //  types = new_types;
  voxels = new_voxels;
  // if (new_copy) voxels_copy = new_copy;
  // if (new_types_copy) types_copy = new_types_copy;
