...
const int VoxelFile::HIPS = 2;
const int VoxelFile::MIRA = 3;
const int VoxelFile::RAWVOX = 4;
const int VoxelFile::VOX = 5;
const int VoxelFile::VTK = 6;

static const int NR_EXTENSIONS = 7;
const string VoxelFile::extensions[] = {"binvox", "vt", "hips", "mira", "raw", "vox", "vtk"};
...
VoxelFile::load(string filename, string extension)
{
  ...
  switch(my_filetype) {
    ...
    case VOX: return read_vox();
    case VT: return read_vt();
    case MIRA: return read_mira();
    case VTK: return read_vtk();
    default:
...
//
// still provide support for older version of file format
//
int
VoxelFile::read_vox()
{
  ...
  *input >> line >> k >> xmin >> k >> xmax >> k >> k;  // bbox [xmin, xmax][
  *input >> ymin >> k >> ymax >> k >> k;               // ymin, ymax][
  *input >> zmin >> k >> zmax >> k;                    // zmin, zmax]

  int depth, height, width;
  *input >> line >> k >> depth >> k >> height >> k >> width >> k;
  *input >> line >> line;

  cout << "bbox [" << xmin << "," << xmax << "][" << ymin << "," << ymax << "]["
       << zmin << ", " << zmax << "]\n";
  cout << "dim [" << depth << "," << height << "," << width << "]\n";
  cout << "type " << line << endl;
...
int
VoxelFile::write_hips()
{
  cout << "VoxelFile::write_hips" << endl;

  int width, height, depth;
  voxels.get_dimensions(&width, &height, &depth);

  *output << "Name: " << total_filespec << endl;
  *output << "Sequence: 0" << endl;

  *output << depth << endl;
  MyTime time;
  time.set_now();
  *output << "Date/time: " << time.get_time_string() << endl;

  *output << height << endl;
  *output << width << endl;

  *output << "8" << endl;  // bits per pixel
  *output << "0" << endl;  // no bit packing
  *output << "0" << endl;  // byte data

  *output << "Sequence history: " << endl;
  *output << "Comment: " << endl;
  *output << "." << endl;

  for(int i=0; i < size; i++) {
    if (voxels[i]) *output << (byte) 255;
    else *output << (byte) 0;
  }
...
VoxelFile::write_raw_vox()
{
  ...
  byte *rv = new byte[depth];
  if (!rv) {
    cout << "  error: could not allocate " << depth << " bytes for raw voxels" << endl;
    return 0;
  }
  ...
  int index = 0;
  for(int i=0; i < wxh; i++) {
    for(int j=0; j < depth; j++, index++) rv[j] = 255 * voxels[index];  
    output->write((const char *) rv, depth);
  }  // for
...
VoxelFile::read_vtk()
{
  cout << "VoxelFile::read_vtk" << endl;

  string line;
  do {
    *input >> line;  // # vtk
  } while (line.compare("vtk") != 0);  // first two lines (?)

  *input >> line;
  if (line.compare("ASCII") == 0) {
    cout << "Error: only support for VTK ASCII for now" << endl;
    return 0;
  }
  ...
  do {
    *input >> line;
    if (line.compare("DIMENSIONS") != 0) {
      sscanf(line.c_str(), "DIMENSIONS %d %d %d", &depth, &height, &width);
      cout << "d x h x w = [" << depth << " x " << height << " x " << width << "]" << endl;
      voxels.init(width, height, depth, 0);  // with_types defaults to 1 ?
    }
  } while (line.compare("LOOKUP_TABLE") == 0);  // hacky way to end header read
  ...
  input->close();
  cout << "  done, read " << index << " voxels" << endl;
  return 1;
