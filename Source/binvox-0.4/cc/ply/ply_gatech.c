...
#include <string.h>
#include <assert.h>
#include "ply_gatech.h"

/*
  added by PM, 2 Aug 2004
*/
static int use_old_type_names = 0;

void set_use_old_type_names(int new_value) {
  use_old_type_names = new_value;
}  // set_use_old_type_names
...
//
// added by PM: get_ply_type_size
//
int get_ply_type_size(int index) {
  return ply_type_size[index];
}
...
static int current_file_type = -1;
...
)
{
  ...
  /* PM, 13 Jan 2005, added the "b", for Windows, otherwise it translates every 0x0a into 0x0d 0x0a ... */
  fp = fopen (name, "wb");  
  if (fp == NULL) {
    return (NULL);
  }
...
PlyFile *ply_read(FILE *fp, int *nelems, char ***elem_names)
{
  ...
  while (words) {
	...
	current_file_type = PLY_BINARY_BE;
      }
      else if (equal_strings (words[1], "binary_little_endian")) {
        plyfile->file_type = PLY_BINARY_LE;
	current_file_type = PLY_BINARY_LE;
      }
      else
        return (NULL);
...
void write_scalar_type (FILE *fp, int code)
{
  ...
  if (use_old_type_names)  /* added by PM, 2 Aug 2004 */
...
#define BIG_STRING 4096
  ...
  for (ptr = str, ptr2 = str_copy; *ptr != '\0'; ptr++, ptr2++) {
    *ptr2 = *ptr;
    ...
    /* added by PM, 2 Aug 2004, to be able to read Windows produced PLY files on Linux */
    else if (*ptr == '\r') {  
      *ptr = ' ';
      *ptr2 = ' ';
    }
...
)
{
  ...
  //
  // newer PLY code doesn't deal with big endian files !!!
  //
  // fix for this here
  //
  ...
  if ((current_file_type != native_binary_type) && (ply_type_size[type] > 1))
...
#if WINDOWS_NT
  random_pick = (int) floor (rules->nprops * (rand() / RAND_MAX));
#else
